...
- 不负责创建组件骨架，相关工作交给 `create-component`。
- 不负责 Demo 页编排，相关工作交给 `create-demo-page`。
- 不负责 Markdown 文档迁移与 API 表维护，相关工作交给 `migrate-component-doc`。

## 使用场景
1.  **代码维护**: 统一现有代码库的风格，提高可读性。
2.  **新功能开发**: 确保新增组件和逻辑符合团队规范。
3.  **Code Review**: 作为代码审查的标准依据。
4.  **组件补注释**: 在新建组件后补齐 `types.ts`、公共函数和类型定义注释。
