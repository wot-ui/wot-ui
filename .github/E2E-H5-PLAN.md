...
更新时间：2026-09-19。当前已登记 101 个组件、95 条路由、371 个独立场景。前 100 个组件、94 条路由、367 个场景完成了连续三轮完整矩阵：3693 次执行，3660 通过、33 明确跳过、0 已知缺陷、0 非预期失败、0 重试；随后合入主分支的 BarCode 已完成 2 个功能场景及新增路由的四配置增量验证，共 14 次通过。H5-001 ～ H5-009 已完成归因、修复或测试纠正。远程 CI 与 Linux 视觉基线仍待最终收口，真实设备不属于本计划范围。
...
- 101 个组件目录全部进入覆盖矩阵；组合子组件在父组件流程中验收，但必须有自己的断言。
- 95 条当前注册页面路由全部登记：2 个主页面、84 个 Demo 入口、9 个额外子页面。H5 编译后可访问的页面全部有独立冒烟用例；条件编译排除项需记录源码依据。
...
| 组件与页面   | 101 个组件、83 个组件 Demo                | 子组件按组合流程覆盖，完整映射见文末                                |
| 路由         | `src/pages.json` 共 95 条                 | 建立路由清单，核实 H5 条件编译和实际 URL                            |
...
| 步骤 | 内容                                                                                                                                                                                                    | 交付与验收                                                                                                   | 状态                                                                                   |
| ---- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------- |
| M0   | 盘点组件、Demo、路由、测试框架和 CI，建立全量矩阵                                                                                                                                                       | 101 个组件与 95 条路由无遗漏；本计划保存入库                                                                 | 已完成                                                                                 |
...
| M9   | 展示与主题：Avatar、Badge、BarCode、Card、Cell、Circle、ConfigProvider、CountDown、CountTo、Divider、Empty、Gap、Grid、Icon、Layout、Loading、Loadmore、NoticeBar、Progress、Skeleton、Table、Tag、Text | 内容、适用的状态切换、溢出和布局；计时可控；主题和语言切换；表格滚动/排序按源码能力核对                      | 原 34 个核心场景三配置三轮验证；BarCode 2 个场景三配置增量通过；101 个组件均有功能映射 |
...
- [x] 当前 101 个组件全部映射到明确场景；新增组件通过清单差异检查及时登记。
- [x] 当前 95 条注册路由全部登记；H5 有效页面的冒烟全部通过，排除项都有源码依据。
...
| 2026-09-19       | 主分支增量 | 补入新合并的 BarCode 组件、路由冒烟和可重新生成的清单守卫                    | `bar-code-ci-fix/results.json`：四配置增量 14 次通过；矩阵现为 101 组件、95 路由、371 场景 | 等待更新后的远程 CI                                    |
...
| wd-bar-code             | [barCode](../src/subPages/barCode/Index.vue)                               | M9   | 独立组件场景           | 已实现 2 个关联核心场景；结果见矩阵  |
